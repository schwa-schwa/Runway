...
function GrowthReportPage() {
  ...
  const radarChartData = useMemo(() => {
    if (
      !averageScores ||
      !averageScores.user_chart_data_averages ||
      !averageScores.overall_chart_data_averages
    )
      return [];
  ...
  const summaryData = useMemo(() => {
    ...
    const scoreChangeFromPrevious =
      attempts > 1
        ? scoreHistory[attempts - 1].score - scoreHistory[attempts - 2].score
        : 0;
    const scoreChangeFromFirst =
      attempts > 0
    ...
    if (Object.keys(userAvgs).length > 0) {
      bestItem = Object.keys(userAvgs).reduce((a, b) =>
        userAvgs[a] > userAvgs[b] ? a : b
      );
      worstItem = Object.keys(userAvgs).reduce((a, b) =>
        userAvgs[a] < userAvgs[b] ? a : b
      );
    }
  ...
  return (
    <Box sx={{ flexGrow: 1, p: 3, background: "#f4f6f8", minHeight: "100vh" }}>
      ...
      {loading && (
        <CircularProgress sx={{ justifyContent: "center", width: "100%" }} />
      )}
            ...
            <Paper elevation={3}>
              ...
              >
                <Box sx={{ flex: 1, p: 2, textAlign: "center" }}>
                  ...
                  <Typography variant="h4" component="p">
                    {summaryData.scoreChangeFromFirst}
                  </Typography>
                </Box>
                <Divider orientation="vertical" flexItem />
                <Box sx={{ flex: 1, p: 2, textAlign: "center" }}>
                  ...
                  <Typography variant="h4" component="p">
                    {summaryData.scoreChangeFromPrevious}
                  </Typography>
                </Box>
                <Divider orientation="vertical" flexItem />
                <Box sx={{ flex: 1, p: 2, textAlign: "center" }}>
                  ...
                  <Typography variant="h5" component="p" sx={{ pt: 1 }}>
                    {summaryData.bestItem}
                  </Typography>
                </Box>
                <Divider orientation="vertical" flexItem />
                <Box sx={{ flex: 1, p: 2, textAlign: "center" }}>
